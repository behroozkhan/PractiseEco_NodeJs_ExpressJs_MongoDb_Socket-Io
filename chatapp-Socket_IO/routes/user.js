...
router.post('/register', async (req, res) => {
    try {
        const validateUserFields = await JoiValidSchema.validateAsync(req.body);
        ...
        const userInfo = { ...savedUser._doc, password: undefined };
        const token = jwt.sign({ _id: savedUser._id, email: savedUser.email, username: savedUser.name }, process.env.JWT_SECRET);
        return res.status(200).send({ status: 200, userInfo, message: chalk.bgGreen("success"), token });
    } catch (error) {
